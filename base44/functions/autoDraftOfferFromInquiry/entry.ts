...
Pravidla návrhu mlžítka: minimalistické, čisté, reálně vyrobitelné. Pro CUSTOM návrh preferuj jednoduchou výrobu: ideálně jeden souvislý trubkový profil nebo co nejméně dílů; pokud možno ohyby v jedné rovině; plynulé rádiusy bez zbytečně těsných nebo složených 3D ohybů; minimum svarů; minimum samostatných ramen; standardní trubky z kalkulační tabulky; patku/kotvení řeš jednoduše a servisovatelně; trysky osazuj přímo do hlavní konstrukce; připojení vody navrhni skrytě a dostupně pro servis. Čistá linie a proporce mají přednost před dekorativní složitostí. Návrh musí být vyrobitelný běžnými procesy dělení, ohýbání trubek, TIG/MIG svařování, broušení/saténování, závitování a testování. Pokud tvar vyžaduje složitou 3D geometrii, navrhni jednodušší ekvivalent se stejným vizuálním významem. Pro kalkulační základ používej standardní profily dostupné v tabulce: jekl 70×70×3 nebo trubky Ø76,1×3; Ø60,3×3,6; Ø52×3; Ø42,4×3,6; Ø40×3; Ø33,7×3. Materiálový základ kalkulace je nerez 1.4301 / AISI 304; pokud zadání výslovně požaduje AISI 316L, uveď requires_316l = true a systém přidá upozornění k přepočtu materiálu. U BENDY jeden štíhlý nerezový profil s jediným plynulým horním obloukem, žádné výhonky, větve, hadice, kabely ani přídavná ramena.\n\nPovolené cost_plan.rate_key hodnoty pro CUSTOM: ${CUSTOM_COST_KEYS.join(', ')}. Cost plan vždy popisuje výrobu JEDNOHO kusu custom produktu; systém jej následně násobí požadovaným počtem kusů. Pro každý použitý klíč vrať realistické množství a krátké zdůvodnění. Nezapočítávej položku, kterou návrh nepotřebuje.`,
    ...
    if (!customMode) {
      ...
      product = {
        ...
        material: clean(customSpec?.material) || null,
        image_url: '',
        gallery_urls: [],
        price_from: 0,
      };
    ...
    const productLock = customMode
      ? `CUSTOM DESIGN LOCK: vycházej z MASTER konceptu a výrobní specifikace. Zachovej přesně hlavní siluetu, počet profilů, počet a směr ohybů, patku/kotvení a polohu trysek. Design nesmí při další vizualizaci „růst“ o nové větve, smyčky, ramena ani dekorace. Tvar má zůstat jednoduchý a vyrobitelný: ${short(customSpec?.design_description, 700)} Profil: ${clean(customSpec?.primary_profile)}. Ohýbání: ${clean(customSpec?.bend_strategy)}. Svařování: ${clean(customSpec?.weld_strategy)}. Trysky: ${clean(customSpec?.nozzle_strategy)}.`
      : isBendy
...
Architektonický styl: klidný, prémiový, realistický, český veřejný nebo zahradní prostor podle zadání. Prvek osaď bezpečně k pěší trase nebo pobytové zóně, ne jako překážku. Fotografie má působit jako skutečná realizace pořízená kvalitním fotoaparátem: přirozené materiály, fyzicky věrohodné stíny, správná perspektiva, žádný renderový plastový vzhled. Bez louží, bez grafických overlayů, bez textů, bez loga, bez nereálných světelných efektů. Výsledek má být použitelný jako vizuální návrh v klientské prezentaci.`,
    ...
    const orderData = {
      ...
      production_notes: customMode ? `CUSTOM KONCEPT — ${clean(customSpec?.name)}\nProfil: ${clean(customSpec?.primary_profile)}\nRozměry: ${clean(customSpec?.dimensions_summary)}\nOhýbání: ${clean(customSpec?.bend_strategy)}\nSvařování: ${clean(customSpec?.weld_strategy)}\nTrysky: ${clean(customSpec?.nozzle_strategy)}\nPřívod vody: ${clean(customSpec?.water_connection_strategy)}\nPostup: ${(customSpec?.manufacture_steps || []).map(clean).filter(Boolean).join(' → ')}`.slice(0, 3000) : '',
      special_requirements: customMode ? `${(customPricing?.warnings || []).join(' ')} ${clean(customSpec?.production_notes)}`.trim().slice(0, 2000) : '',
    ...
    for (let i = 0; i < generatedVisualizations.length; i += 1) {
      ...
      try {
        ...
        const offerVariant = await base44.asServiceRole.entities.OfferVariant.create({
          ...
          line_items: unitPrice > 0 ? (customMode && customPricing?.line_items?.length
            ? customPricing.line_items.map((line) => ({ ...line, quantity: Number(line.quantity || 0) * variant.quantity, total: Number(line.total || 0) * variant.quantity, source: customPricing.source }))
            : [{ label: product.name, quantity: variant.quantity, unit: 'ks', unit_price: unitPrice, total: unitPrice * variant.quantity, source: pricingSource === 'mlzny_disk' ? pricing.source : 'Product.price_from' }]) : [],
    ...
    return Response.json({
      ...
      product_mode: customMode ? 'custom' : 'catalog',
      product_id: product.id,
      product_slug: product.slug || '',
      product_name: product.name,
      product_price_from: unitPrice,
      custom_product: customMode ? { ...customSpec, master_image_url: customMasterUrl, estimated_unit_price_ex_vat: unitPrice, pricing_basis: customPricing?.material_basis || '', pricing_warnings: customPricing?.warnings || [] } : null,
      custom_master_asset: customMasterAsset,
      custom_pricing: customPricing,
      pricing: {
        source: pricingSource,
        source_label: pricingSource === 'custom_sheet_estimate' ? `${customPricing?.source || 'MLŽNÝ DISK'} · zakázkový výrobní odhad` : pricingSource === 'mlzny_disk' ? pricing.source : pricingSource === 'catalog' ? 'Katalog Product.price_from' : 'Ruční nacenění',
      ...
      requested_variants: variantSpecs.map((variant) => ({ ...variant, price: unitPrice > 0 ? unitPrice * variant.quantity : 0, price_source: pricingSource, pricing_label: pricingSource === 'custom_sheet_estimate' ? `${customPricing?.source || 'MLŽNÝ DISK'} · zakázkový výrobní odhad` : pricingSource === 'mlzny_disk' ? pricing.source : pricingSource })), 
      ...
      ai_content: {
        ...
        solution_summary: `${solutionSummary}${customMode ? ` Zakázkový koncept je navržen s důrazem na jednoduchou výrobu: ${clean(customSpec?.primary_profile)}, ${clean(customSpec?.bend_strategy)}, ${clean(customSpec?.weld_strategy)}. Před výrobou proběhne technické potvrzení rozměrů, rádiusů, kotvení a materiálu.` : ''}${variantSpecs.length ? ` Nabídka počítá s variantami: ${variantSpecs.map((variant) => `${variant.label}${unitPrice > 0 ? ` — ${new Intl.NumberFormat('cs-CZ').format(unitPrice * variant.quantity)} Kč bez DPH` : ''}`).join('; ')}.` : ''}`,
        benefits,
