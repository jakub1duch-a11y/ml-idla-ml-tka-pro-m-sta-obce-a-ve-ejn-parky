...
export default function ContentPlanList({ posts, onChange }) {
  ...
  const regenerateImage = async () => {
    ...
    try {
      const ratio = form.platform === 'instagram' ? (form.post_format === 'story' || form.post_format === 'reel' ? 'vertikální 9:16 kompozice' : 'portrétní 4:5 kompozice 1080 × 1350') : form.platform === 'blog' ? 'široká 16:9 editorial kompozice' : 'široká reklamní 1.91:1 kompozice';
      const res = await base44.integrations.Core.GenerateImage({ prompt: `Prémiový fotorealistický marketingový vizuál MLŽIDLA.cz k tématu „${form.title}“. ${ratio}. Zachovej přesnou geometrii zvoleného reálného produktu: proporce, počet a polohu trubek, ramen, trysek, patku, ukotvení a nerez AISI 316L. Nevymýšlej nové konstrukční prvky. Moderní český nebo evropský veřejný prostor nebo autentická výrobní či montážní scéna podle zadání. Přirození lidé pro měřítko; u výroby realistické ruce, vhodné ochranné pomůcky a bezpečná práce. Jemná průsvitná mlha 50–100 μm jen tam, kde dává smysl. Bez generického stock vzhledu, bez vodoznaku, bez textu a bez falešného loga v obraze. Brand: Deep Steel #0D2D38, Ocean Teal #0E5B67, Mist Aqua #61D5E5, Living Green #6F8F72 a bílá. Zelená evokuje stromy, stín a úlevu ve veřejném prostoru, ne neověřené ekologické tvrzení. ${form.visual_prompt || ''}` });
