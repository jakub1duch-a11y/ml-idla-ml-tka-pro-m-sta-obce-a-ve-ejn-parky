...
export default function BrandShowcase() {
  return <>
    <section className="bg-white"><div className="mx-auto grid max-w-7xl gap-10 px-6 py-16 lg:grid-cols-[1.1fr_.9fr] lg:px-10 lg:py-24">
      <div><p className="font-mono text-[11px] tracking-[.18em] uppercase text-secondary">Základ identity</p><h2 className="mt-4 font-heading text-4xl text-foreground lg:text-5xl">Barvy, znak a typografie, které drží značku pohromadě.</h2><p className="mt-5 max-w-xl text-muted-foreground">MLŽIDLA® spojují průmyslovou přesnost, autorskou tvorbu a péči o výsledek. Vizuální systém pracuje s hlubokou modří, jasným cyan akcentem, světlou mlhou a nerezovými detaily.</p><div className="mt-8 grid grid-cols-5 overflow-hidden rounded-2xl border border-slate-200 shadow-sm">{[['#071A2F','Deep Ink'], ['#0878E8','Ocean Blue'], ['#00B7FF','Fresh Cyan'], ['#7DD3FC','Mist Blue'], ['#F5FAFD','Ice White']].map(([color, name]) => <div key={name}><div className="h-16" style={{ backgroundColor: color }}/><p className="p-2 text-[9px] font-mono uppercase text-slate-500">{name}</p></div>)}</div></div>
      <div className="rounded-[28px] border border-slate-200 bg-gradient-to-br from-white to-mist/70 p-8 shadow-sm"><img src="https://media.base44.com/images/public/6a3ee88c10959cd3588c4d68/314f4a3ac_mlzidla_logo_bez_pozadi.png" alt="Logo MLŽIDLA" className="mx-auto h-24 w-auto max-w-full object-contain"/><div className="mt-8 border-t border-slate-200 pt-6"><p className="font-mono text-[11px] tracking-[.18em] uppercase text-secondary">Typografie</p><p className="mt-3 font-heading text-2xl text-foreground">Manrope — titulky a logotyp</p><p className="text-muted-foreground">Inter — texty · JetBrains Mono — technická data · Caveat — podpis „Architektura, která dýchá.“</p></div><div className="mt-6 overflow-hidden rounded-2xl bg-primary shadow-lg"><video className="aspect-video w-full object-cover" src="/media/optimized/e93fc3844_mist-animatedicon02.webm" autoPlay muted loop playsInline /><span className="sr-only"><Play/> Animace značky MLŽIDLA</span></div></div>
    </div></section>
