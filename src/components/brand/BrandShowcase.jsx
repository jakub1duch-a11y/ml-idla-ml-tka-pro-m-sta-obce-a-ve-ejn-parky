...
export default function BrandShowcase() {
  return <>
    <section className="bg-white"><div className="mx-auto grid max-w-7xl gap-10 px-6 py-16 lg:grid-cols-[1.1fr_.9fr] lg:px-10 lg:py-24">
      <div><p className="font-mono text-[11px] tracking-[.18em] uppercase text-secondary">Základ identity</p><h2 className="mt-4 font-heading text-4xl text-foreground lg:text-5xl">Barvy, znak a typografie, které drží značku pohromadě.</h2><p className="mt-5 max-w-xl text-muted-foreground">MLŽIDLA® propojují technickou přesnost nerezu s pocitem osvěžení. Znak kapky a oblouku odkazuje na architekturu, pohyb i jemnou mlhu.</p><div className="mt-8 grid grid-cols-5 overflow-hidden rounded-lg border border-slate-200">{[['#082430','Deep Steel'], ['#0E5C66','Ocean Teal'], ['#58C7D6','Mist Aqua'], ['#B9C3C8','Stainless'], ['#F6F8F8','White Mist']].map(([color, name]) => <div key={name}><div className="h-16" style={{ backgroundColor: color }}/><p className="p-2 text-[9px] font-mono uppercase text-slate-500">{name}</p></div>)}</div></div>
