      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
          ...
          <h2 className="font-heading font-light text-3xl lg:text-4xl text-white tracking-tight mb-4">
            Vaše mlžítko, <span className="italic text-cyan">chytřejší než kdy dřív.</span>
          </h2>
          ...
          <ul className="space-y-3 mb-8">
            ...
            <li key={b} className="flex items-start gap-2.5 text-sm text-white/70">
                <CheckCircle2 size={16} className="text-cyan shrink-0 mt-0.5" /> {b}
              </li>
            )}
          </ul>
          <Link to="/kontakt?produkt=Smart%20App%20řízení"
            onClick={() => trackQuickInquiryClick('Smart App řízení', 'smart_control_promo')}
            className="inline-flex items-center gap-2 px-7 py-3.5 bg-cyan text-slate-900 text-sm font-bold rounded-full hover:bg-cyan/90 transition-all">
