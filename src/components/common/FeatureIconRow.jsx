    ...
    <div className={`grid grid-cols-2 sm:grid-cols-4 gap-4 ${className}`}>
      ...
      <motion.div
        ...
        className="flex flex-col items-center text-center gap-2 p-4 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="font-medium leading-tight text-slate-900 text-2xl">{item.label}</p>
          {item.value && <p className="text-slate-500 leading-tight text-xl uppercase">{item.value}</p>}
        </motion.div>
      )}
    </div>);
