...
export default function CoolingCardEffect({ active = false }) {
  ...
  return <div ref={effectRef} className="pointer-events-none absolute inset-0 z-0 overflow-hidden" aria-hidden="true">
    ...
    <div className="cooling-texture" />
    <div className={`cooling-droplets ${cooled ? 'opacity-100' : 'opacity-0'} transition-opacity duration-700`}><i /><i /><i /><i /></div>
    ...
    {cooled && <div className="absolute flex gap-1.5 text-cyan animate-bounce right-10 top-10 hidden"><i className="h-1.5 w-1.5 rounded-full bg-current" /><i className="mt-2 h-1 w-1 rounded-full bg-current" /><i className="h-1.5 w-1.5 rounded-full bg-current" /></div>}
  </div>;
}