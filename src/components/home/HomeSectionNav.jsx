...
export default function HomeSectionNav() {
  ...
  return <nav aria-label="Navigace hlavní stránkou" className={`fixed inset-x-0 top-[72px] z-30 border-y border-slate-200 bg-white/95 backdrop-blur-xl transition-all duration-300 ${visible ? 'translate-y-0 opacity-100' : '-translate-y-4 pointer-events-none opacity-0'}`}><div className="flex w-full gap-1 overflow-x-auto px-3 py-2.5 lg:justify-center lg:px-10 [&::-webkit-scrollbar]:hidden hidden">{SECTIONS.map(({ id, label }) => <a key={id} href={`#${id}`} onClick={(event) => scrollTo(event, id)} aria-current={activeId === id ? 'true' : undefined} className={`relative shrink-0 px-3 py-2 text-xs font-bold transition sm:px-4 sm:text-sm ${activeId === id ? 'text-slate-950' : 'text-slate-500 hover:text-slate-950'}`}>{label}{activeId === id && <span className="absolute inset-x-3 bottom-0 h-0.5 bg-cyan sm:inset-x-4" />}</a>)}</div></nav>;
}