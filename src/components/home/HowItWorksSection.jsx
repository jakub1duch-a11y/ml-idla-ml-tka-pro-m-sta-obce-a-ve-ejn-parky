    ...
    <section className="py-24 bg-surface">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-16 text-center">
          <p className="text-xs font-mono tracking-widest uppercase text-cyan mb-3">Technologie evaporace</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {features.map((f, i) => (
            <motion.div
              ...
              className="group p-8 rounded-2xl bg-card_bg border border-white/10 hover:border-cyan/30 transition-all"
            >
              {/* Icon */}
              <div className="w-14 h-14 rounded-xl bg-cyan/10 border border-cyan/20 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                <f.icon size={28} className="text-cyan" />
              </div>

              {/* Content */}
              <h3 className="font-heading font-light text-xl text-white tracking-tight mb-3">
                {f.title}
              </h3>
