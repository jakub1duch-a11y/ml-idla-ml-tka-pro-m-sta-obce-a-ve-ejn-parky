...
import { Droplets, Gauge, Snowflake, Wind } from 'lucide-react';
...
export default function MistStoryScene({ scene }) {
  ...
  return <motion.article ref={ref} initial={{ opacity: 0.25, scale: 0.98 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ amount: 0.35 }} transition={{ duration: 0.7 }} className="relative min-h-[78svh] overflow-hidden border-y border-white/10 bg-slate-950 text-white">
    ...
    <video key={video} src={video} autoPlay muted loop playsInline preload="auto" className={`absolute inset-0 h-full w-full object-cover transition-all duration-700 ${isHeatScene && !cooled ? 'scale-110 saturate-150 sepia-[.18]' : 'scale-100 opacity-75'}`} />
    <div className={`absolute inset-0 transition-all duration-700 ${isHeatScene && !cooled ? 'bg-gradient-to-r from-orange-950/90 via-red-950/62 to-slate-950/75' : 'bg-gradient-to-r from-slate-950/88 via-cyan-950/52 to-slate-950/70'}`} />
    ...
    <div className="relative z-10 mx-auto flex min-h-[78svh] max-w-7xl items-end px-6 py-14 lg:px-10 lg:py-20"><motion.div whileInView={{ y: [32, 0] }} viewport={{ once: true }} transition={{ duration: 0.65 }} className="max-w-2xl"><p className={`mb-5 text-xs font-bold uppercase tracking-[0.22em] ${isHeatScene && !cooled ? 'text-orange-200' : 'text-cyan'}`}>{scene.number} · {cooled ? 'Ochladivý přechod' : scene.eyebrow}</p><h2 className="font-heading text-4xl font-medium leading-[1.02] tracking-tight text-white lg:text-6xl">{title}</h2><p className="mt-5 max-w-xl text-base leading-relaxed text-white/75 lg:text-lg">{text}</p>{isHeatScene && cooled ? <div className="mt-8 flex flex-wrap gap-3">{COOLING_STEPS.map((step, index) => <motion.span key={step} initial={{ opacity: 0, x: -18 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.18 }} className="inline-flex items-center gap-2 rounded-full border border-cyan/40 bg-cyan/10 px-3 py-2 text-xs font-semibold text-cyan"><Snowflake size={14} />{step}</motion.span>)}</div> : <div className="mt-8 flex flex-wrap gap-x-6 gap-y-3">{scene.tags.map((tag, index) => { const Icon = [Gauge, Droplets, Wind][index % 3]; return <motion.span key={tag} whileHover={{ x: 3 }} className="inline-flex items-center gap-2 text-xs font-semibold text-white/90"><Icon size={16} className={`${isHeatScene ? 'text-orange-200' : 'text-cyan'} drop-shadow-[0_0_10px_rgba(103,232,249,.75)]`} />{tag}</motion.span>; })}</div>}</motion.div></div>
  </motion.article>;
}