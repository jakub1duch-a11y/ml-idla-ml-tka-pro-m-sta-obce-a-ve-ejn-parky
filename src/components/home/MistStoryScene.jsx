...
export default function MistStoryScene({ scene }) {
  ...
  return <motion.article ref={ref} initial={{ opacity: 0.25, scale: 0.98 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ amount: 0.35 }} transition={{ duration: 0.7 }} className="relative min-h-[78svh] overflow-hidden border-y border-white/10 bg-slate-950 text-white">
    ...
    <div className={`absolute inset-0 transition-all duration-700 hidden ${isHeatScene && !cooled ? 'bg-gradient-to-r from-orange-950/90 via-red-950/62 to-slate-950/75' : hasPhoto ? 'bg-gradient-to-r from-slate-950/95 via-slate-950/52 to-slate-950/75' : 'bg-gradient-to-r from-slate-950/88 via-cyan-950/52 to-slate-950/70'}`} />
    ...
    {cooling && <><div className="mist-scene-cloud" /><div className="mist-particles"><i /><i /><i /><i /></div><div className="absolute bg-[size:7px_7px] mix-blend-screen inset-5 opacity-45 bg-[radial-gradient(rgba(207,250,254,.25)_0.4px,transparent_0.7px)] hidden" /></>}
