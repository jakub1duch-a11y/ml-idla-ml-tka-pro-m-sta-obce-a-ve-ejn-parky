...
export default function QuickBenefits({ className = '', compact = false, limit }) {
  ...
  return <div className={`grid gap-3 ${className}`}>{benefits.map((benefit) => <motion.article key={benefit.value} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.25 }} className={`rounded-3xl border border-white/25 bg-white/10 p-5 shadow-2xl shadow-slate-950/10 backdrop-blur-xl ${compact ? 'flex items-center gap-4' : ''}`}><div className="relative flex h-24 w-24 shrink-0 items-center justify-center rounded-2xl border border-white/30 bg-white/15 text-cyan shadow-inner shadow-white/10">{benefit.aura && <motion.span animate={{ scale: [0.75, 1.2, 0.75], opacity: [0.15, 0.6, 0.15] }} transition={{ duration: 3, repeat: Infinity }} className="absolute inset-1 rounded-xl bg-cyan blur-xl" />}<motion.div animate={benefit.animation} transition={{ duration: benefit.duration || 3.4, repeat: Infinity, ease: 'easeInOut' }} className="relative"><benefit.icon size={compact ? 46 : 82} strokeWidth={1.5} /></motion.div></div><div className={compact ? '' : 'mt-5'}><h3 className="font-heading text-xl font-semibold text-white">{benefit.value}</h3><p className="mt-1 text-sm leading-relaxed text-white/70">{benefit.text}</p></div></motion.article>)}</div>;
}