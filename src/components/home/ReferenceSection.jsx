...
function Lightbox({ images, initialIndex, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-ink/95 backdrop-blur-lg flex items-center justify-center" onClick={onClose}>
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-white/5">
        ...
        {/* Gallery count */}
        {allImages.length > 1 && (
          <div className="absolute bottom-4 right-4 flex items-center gap-1.5 px-2.5 py-1 bg-ink/70 backdrop-blur-sm border border-white/10 rounded-full text-[10px] font-mono text-white/60">
      ...
      <div className="p-6">
        ...
        <h3 className="font-heading font-light text-lg text-white tracking-tight mb-2 group-hover:text-cyan/90 transition-colors leading-snug">
          {project.name}
        </h3>
        {project.description && (
...
export default function ReferenceSection() {
  ...
  return (
    <section className="py-24 bg-ink" id="reference">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        {/* Header */}
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-14">
          <p className="text-xs font-mono tracking-widest uppercase text-cyan mb-3">Reference</p>
