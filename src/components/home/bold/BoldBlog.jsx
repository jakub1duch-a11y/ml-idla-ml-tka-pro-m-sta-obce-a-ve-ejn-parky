...
function PostCard({ post, i }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
      <Link to={`/blog/${post.slug || post.id}`} className="group relative block h-72 overflow-hidden border-2 border-white/10 hover:border-red-600 transition-colors">
        <img src={bg} alt={post.title} className="absolute inset-0 w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-500" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-black/10" />
        <div className="relative h-full flex flex-col justify-end p-6">
          <span className="inline-block self-start text-[10px] font-black text-white tracking-widest uppercase px-2.5 py-1 bg-red-600 mb-3">
            {CATEGORY_LABELS[post.category] || post.category || 'Článek'}
          </span>
          <h3 className="font-heading font-black uppercase text-lg text-white tracking-tight leading-snug mb-2 line-clamp-2">{post.title}</h3>
          <p className="text-sm text-white/60 font-light leading-relaxed line-clamp-2 mb-3">{post.perex}</p>
          <div className="flex items-center gap-2 text-xs text-white font-black uppercase group-hover:gap-3 transition-all">
...
export default function BoldBlog() {
  ...
  return (
    <section className="py-20 lg:py-28 bg-black border-t-2 border-white/10">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-12 flex flex-col lg:flex-row lg:items-end justify-between gap-4">
          <div>
            <p className="text-xs font-black tracking-widest uppercase text-red-600 mb-3">Novinky & znalosti</p>
            <h2 className="font-heading font-black uppercase text-4xl lg:text-5xl text-white tracking-tight">Co je nového u nás</h2>
          </div>
          <Link to="/blog" className="inline-flex items-center gap-2 text-sm text-white/70 font-bold uppercase hover:text-white hover:gap-3 transition-all">
            Všechny články <ArrowRight size={14} />
          </Link>
        </motion.div>

        {loading ? (
          <div className="flex justify-center py-14"><Loader size={22} className="animate-spin text-white/30" /></div>
        ) : (
