    ...
    <section className="py-24 lg:py-32 bg-slate-50">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <div>
            <p className="text-[11px] tracking-widest uppercase text-teal-600 mb-2">Realizace</p>
          ...
          <Link to="/reference" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-slate-900 font-medium transition-colors">
            ...
            {projects.map((p, i) => {
              ...
              return (
                <motion.div key={p.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}
                  ...
                  <Link to={`/reference/${p.id}-${slugify(p.name)}`} className="group relative block w-full h-full rounded-3xl overflow-hidden bg-slate-100">
                    {p.image_url ? (
                      <img src={p.image_url} alt={p.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" loading="lazy" />
                    ) : (
                    ...
                    {isHighlighted && (
                      <span className="absolute top-4 left-4 inline-flex items-center gap-1.5 bg-white text-slate-900 text-[10px] font-medium tracking-widest uppercase px-2.5 py-1 rounded-full">
                        <Sparkles size={11} /> Vybraná realizace
                      </span>
                    )}
                    <div className="absolute bottom-0 left-0 right-0 p-4 lg:p-5">
                      {p.location && (
                        <div className="flex items-center gap-1.5 text-white/70 text-[11px] mb-1">
                          <MapPin size={10} /> {p.location}
                        </div>
                      )}
                      <h3 className={`font-heading font-medium text-white tracking-tight leading-snug ${isHighlighted ? 'text-xl lg:text-2xl' : 'text-sm'} line-clamp-2`}>
                        {p.name}
                      </h3>
