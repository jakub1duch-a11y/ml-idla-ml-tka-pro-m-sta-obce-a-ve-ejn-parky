...
export function GroundScrewIcon() {
  return <svg viewBox="0 0 64 64" className="h-20 w-20"><motion.g initial={{ opacity: 0, y: -6 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}><path {...iconProps} d="M20 15h24M32 15v34m-12-15h24M24 49l8 9 8-9" /><motion.path {...iconProps} d="M25 22h14" animate={{ opacity: [0.35, 1, 0.35] }} transition={{ duration: 1.8, repeat: Infinity }} /></motion.g></svg>;
}

export function AtmosphereIcon() {
  return <svg viewBox="0 0 64 64" className="h-20 w-20"><motion.g initial={{ opacity: 0, scale: 0.7 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}><path {...iconProps} d="m32 9 3 15 15 3-15 3-3 15-3-15-15-3 15-3 3-15Z" /><motion.circle cx="51" cy="15" r="2" fill="currentColor" animate={{ opacity: [0.25, 1, 0.25] }} transition={{ duration: 1.6, repeat: Infinity }} /><circle cx="14" cy="48" r="2" fill="currentColor" /></motion.g></svg>;
}
