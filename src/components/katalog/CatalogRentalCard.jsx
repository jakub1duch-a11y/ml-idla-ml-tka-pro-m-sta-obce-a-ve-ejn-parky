  ...
  return <section className="bg-slate-50 py-16 lg:py-24">
    <div className="site-container overflow-hidden rounded-3xl border border-slate-200 bg-slate-950 text-white">
      <div className="grid lg:grid-cols-2">
        <div className="p-8 lg:p-14"><p className="content-eyebrow mb-4 text-cyan">Pronájem mlžítek</p><h2 className="content-title text-[hsl(var(--card))]">Mlžné osvěžení pro letní akce a eventy.</h2><p className="mt-5 text-white/60">Pošlete nám svou představu. Cena pronájmu zahrnuje návrh počtu zařízení, rozmístění a provozu. Vytvoříme mlžící zónu a zajistíme technika i podporu po celou dobu konání.</p><ul className="mt-7 space-y-3">{points.map((point) => <li key={point} className="flex items-center gap-3 text-sm text-white/75"><Check size={16} className="text-cyan" />{point}</li>)}</ul><div className="mt-8 flex flex-wrap gap-3"><Link to="/poptavka?produkt=Pronájem%20mlžítek" className="btn-metallic-mist px-7 py-4 text-sm font-bold">Poptat termín pronájmu <ArrowRight size={16} /></Link><Link to="/pronajem" className="inline-flex items-center rounded-full border border-white/30 px-7 py-4 text-sm font-bold text-white">Detail pronájmu</Link></div></div>
