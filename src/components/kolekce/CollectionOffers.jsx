...
export default function CollectionOffers() {
  return <section className="mx-auto max-w-7xl px-6 py-16 lg:px-10 lg:py-24">
    ...
    <div className="grid gap-5 lg:grid-cols-3">{OFFERS.map((offer) => <Link key={offer.title} to={`/${offer.slug}`} className="group overflow-hidden rounded-2xl border border-border bg-card transition-all duration-300 hover:-translate-y-1 hover:border-secondary/40 hover:shadow-xl">
      ...
      <div className="p-6"><p className="mb-3 font-mono text-[10px] uppercase tracking-[.16em] text-secondary">{offer.eyebrow}</p><h3 className="font-heading text-2xl text-foreground">{offer.title}</h3><p className="mt-3 text-base leading-relaxed text-muted-foreground">{offer.text}</p><span className="btn-secondary-outline mt-6 inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold text-foreground hidden">Prohlédnout produkty <ArrowRight size={15} /></span></div>
    </Link>)}</div>
  </section>;
}