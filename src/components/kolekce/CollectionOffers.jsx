...
export default function CollectionOffers() {
  return <section className="max-w-7xl mx-auto px-6 lg:px-8 py-16 lg:py-24">
    ...
    <div className="grid gap-5 lg:grid-cols-3">{OFFERS.map((offer) => <Link key={offer.title} to={offer.slug === 'mestske-mlzitka' ? '/mestske-mlzitka' : offer.slug === 'zahradni-mlzitka' ? '/zahradni-mlzitka' : `/kolekce/${offer.slug}`} className="group border border-[#b9c3c8] bg-card overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:border-secondary hover:shadow-xl">
