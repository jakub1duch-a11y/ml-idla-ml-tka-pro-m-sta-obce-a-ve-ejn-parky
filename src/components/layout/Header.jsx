...
export default function Header() {
  ...
  return <><header className={`fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[hsl(var(--foreground))] backdrop-blur-xl ${scrolled ? 'shadow-xl shadow-slate-950/25' : ''}`}><div className="mx-auto flex h-[72px] max-w-[1600px] items-center gap-4 px-5 xl:px-8"><div className="flex shrink-0 self-stretch items-center"><Logo size="sm" /></div><HeaderNav /><div className="ml-auto flex items-center gap-2"><a href="tel:+420774700390" className="hidden 2xl:inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold text-white/75 transition hover:bg-white/10 hover:text-white"><Phone size={15} /> Zavolejte nám</a><a href="tel:+420774700390" aria-label="Zavolejte nám" className="flex h-11 w-11 items-center justify-center text-white xl:hidden"><Phone size={20} /></a><button onClick={() => setMobileOpen((value) => !value)} aria-label={mobileOpen ? 'Zavřít menu' : 'Otevřít menu'} className="flex h-11 w-11 items-center justify-center text-white xl:hidden">{mobileOpen ? <X size={23} /> : <Menu size={23} />}</button></div></div></header><MobileMenu open={mobileOpen} onClose={() => setMobileOpen(false)} /></>;
}