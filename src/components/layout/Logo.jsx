...
export default function Logo({ size = 'md' }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, filter: 'blur(10px)' }} animate={{ opacity: 1, filter: 'blur(0px)' }} transition={{ duration: 1.2, ease: [0.4, 0, 0.2, 1] }} className="inline-flex items-center gap-2 text-white whitespace-nowrap leading-none">
      <span className="flex flex-col gap-0.5"><span className={`${compact ? 'text-base' : 'text-lg'} font-heading font-semibold tracking-tight`}><span className="text-cyan">mlž</span>idla,</span><span className="ml-2 text-[9px] font-medium tracking-[0.14em] text-white/70">mlžítka</span></span>
