...
export default function Logo({ size = 'md' }) {
  ...
  return <motion.div initial={{ opacity: 0, filter: 'blur(8px)' }} animate={{ opacity: 1, filter: 'blur(0px)' }} transition={{ duration: 0.7 }} className="inline-flex items-center gap-2.5 text-primary">
    <span className={`${compact ? 'h-7 w-7' : 'h-10 w-10'} shrink-0 overflow-hidden`}>
      <img src={LOGO_URL} alt="" className={`${compact ? 'h-7 w-[124px]' : 'h-10 w-[168px]'} max-w-none object-contain object-left`} />
    </span>
