...
export default function MobileMenu({ open, onClose }) {
  return (
    <AnimatePresence>{open && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40 bg-slate-950 lg:hidden flex flex-col pt-16">
      <div className="flex items-center justify-between px-6 h-16 border-b border-white/10 shrink-0"><Link to="/" onClick={onClose} className="bg-black/75 rounded-lg px-3 py-2"><Logo size="sm" /></Link><button onClick={onClose} aria-label="Zavřít menu" className="w-11 h-11 flex items-center justify-center rounded-full text-white hover:bg-white/10"><X size={21} /></button></div>
