...
export default function MlzisteHero() {
  ...
  return (
    <div className="relative h-[88vh] min-h-[560px] overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-slate-950/60 via-slate-950/5 to-transparent" />

      <div className="absolute bottom-0 left-0 right-0 max-w-7xl mx-auto px-6 lg:px-10 pb-16 lg:pb-24">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          <span className="inline-flex items-center px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-xs font-mono tracking-widest uppercase text-white/80 mb-5">
            Mlžiště &amp; mlžné zóny na míru
          </span>
          ...
          <button onClick={scrollToConfig}
            className="inline-flex items-center gap-2 px-8 py-4 bg-white text-slate-900 text-sm font-bold rounded-full hover:bg-slate-100 transition-all shadow-xl shadow-black/20">
