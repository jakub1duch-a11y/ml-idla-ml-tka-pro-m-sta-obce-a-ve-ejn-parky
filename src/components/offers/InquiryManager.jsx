            ...
            <div className="p-4 sm:p-6">
              <div className="mx-auto max-w-2xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                ...
                <div className="p-6">
                  ...
                  {attachments.some((item) => item.asset_type === 'generated_visualization') && <div className="mt-5 rounded-2xl bg-[#0d2d38] p-4"><p className="text-[10px] uppercase tracking-[.12em] text-[#61d5e5]">Vizualizace návrhu</p><p className="mt-1 text-[11px] leading-5 text-white/60">Tyto náhledy se zobrazí i v klientském e-mailu a jejich plné soubory budou přiložené.</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{attachments.filter((item) => item.asset_type === 'generated_visualization').slice(0, 3).map((item) => <div key={item.id || item.file_url} className="overflow-hidden rounded-xl bg-white"><img src={item.file_url} alt={item.title || 'Vizualizace projektu'} className="aspect-[16/10] w-full object-cover"/><p className="truncate px-3 py-2 text-[10px] font-semibold text-slate-700">{item.title || 'Vizualizace projektu'}</p></div>)}</div></div>}
