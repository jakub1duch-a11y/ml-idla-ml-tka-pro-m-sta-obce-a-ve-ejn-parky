...
export default function InquiryManager({ inquiries, products, offerProfiles = [], mediaFiles, projectOrders = [], onSent }) {
  ...
  const autoPrepareFromText = async () => {
    ...
    try {
      ...
      const autoVariants = rawVariants.slice(0, 1).map((variant, index) => {
      ...
      await prepareOffer({
        ...
        clientContent: result.ai_content ? {
          ...result.ai_content,
          solution_summary: result.ai_content.solution_summary || '',
        } : null,
        ...
        <div className="border border-border p-5 lg:p-7">
          ...
          <div className="mt-6 rounded-2xl border border-cyan-200 bg-cyan-50/60 p-5 sm:p-6">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
              <div className="max-w-3xl">
                ...
                <h3 className="mt-1 font-heading text-2xl text-slate-950">Vytvořit kompletní obchodní nabídku</h3>
                <p className="mt-2 text-sm leading-relaxed text-slate-600">Jeden krok připraví doporučené řešení, cenu, projektové vizualizace, kompletní PDF a hotový koncept zprávy klientovi. PDF se vždy uloží na sdílený <strong>Mlžný disk</strong>. Nic se klientovi automaticky neodesílá.</p>
                <div className="mt-4 grid gap-2 text-[11px] text-slate-600 sm:grid-cols-4">
                  {['1. Doporučené řešení', '2. Vizualizace', '3. PDF → Mlžný disk', '4. Koncept zprávy'].map((step) => <span key={step} className="rounded-lg border border-cyan-100 bg-white px-3 py-2 font-semibold">{step}</span>)}
                </div>
              </div>
              <button type="button" onClick={autoPrepareFromText} disabled={busy === 'auto-offer' || !selected?.message} className="inline-flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-full bg-[#0e5b67] px-6 py-3.5 text-sm font-bold text-white shadow-sm transition hover:bg-[#0b4b56] disabled:opacity-50"><Sparkles size={17}/>{busy === 'auto-offer' ? 'Vytvářím kompletní nabídku…' : 'Vytvořit kompletní nabídku'}</button>
            </div>
            {busy === 'auto-offer' && <div className="mt-5 rounded-xl border border-cyan-200 bg-white px-4 py-3 text-xs leading-relaxed text-cyan-900"><strong>Vytvářím kompletní nabídku.</strong> Kontroluji zadání a cenu → vytvářím vizualizace → generuji PDF → ukládám na Mlžný disk → připravuji koncept zprávy klientovi.</div>}
          </div>
          ...
          <div className="rounded-2xl border border-cyan-200 bg-cyan-50/40 p-5">
            ...
            <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-5">
              <label className="flex cursor-pointer items-start gap-3">
                <input type="checkbox" checked={suplaAiEnabled} onChange={(event) => { setSuplaAiEnabled(event.target.checked); resetPrepared(); }} disabled={!suplaAiPricing?.ok} className="mt-1 h-4 w-4"/>
                <span className="min-w-0"><strong className="text-sm text-slate-950">SUPPLA AI · pokročilá automatizace a provozní dashboard</strong><span className="mt-1 block text-xs leading-relaxed text-slate-500">Rozšířený projekt: SUPLA Cloud API / self-hosted server, MQTT/API integrace, provozní logika, spotový trh, dashboard, testování a předání dokumentace.</span></span>
              </label>
              {suplaAiPricingError && <p className="mt-3 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-[11px] text-rose-700">Cenový systém SUPPLA není dostupný: {suplaAiPricingError}</p>}
              {suplaAiPricing?.ok && <div className="mt-4">
                <div className="flex flex-wrap gap-2 text-[10px] text-slate-500"><span className="rounded-full bg-cyan-100 px-2.5 py-1 text-cyan-900">{suplaAiPricing.package_name}: <strong>{money(suplaAiPricing.package_price_ex_vat)} Kč bez DPH</strong></span><span className="rounded-full bg-slate-100 px-2.5 py-1">Marže v kalkulaci: {suplaAiPricing.margin_percent}%</span><span className="rounded-full bg-slate-100 px-2.5 py-1">Zdroj: {suplaAiPricing.source}</span></div>
                <div className="mt-4 grid gap-3 lg:grid-cols-3">{(suplaAiPricing.phases || []).map((phase) => <div key={phase.key} className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-[10px] font-semibold uppercase tracking-wider text-cyan-700">{phase.timing}</p><h4 className="mt-2 text-sm font-bold text-slate-900">{phase.title}</h4><p className="mt-2 text-[11px] leading-5 text-slate-500">{phase.scope}</p><p className="mt-3 text-[10px] font-semibold text-slate-600">{phase.price_note}</p></div>)}</div>
                <div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-cyan-100 bg-cyan-50 p-3"><p className="text-[9px] uppercase tracking-wider text-cyan-700">Standardní balíček</p><p className="mt-1 text-sm font-bold text-cyan-950">{money(suplaAiPricing.package_price_ex_vat)} Kč bez DPH</p></div><div className="rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="text-[9px] uppercase tracking-wider text-slate-500">THW‑01 · volitelně</p><p className="mt-1 text-sm font-bold text-slate-900">+ {money(suplaAiPricing.optional_thw01_ex_vat)} Kč bez DPH</p></div><div className="rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="text-[9px] uppercase tracking-wider text-slate-500">Premium varianta</p><p className="mt-1 text-sm font-bold text-slate-900">{money(suplaAiPricing.premium_package_price_ex_vat)} Kč bez DPH</p></div></div>
                <p className="mt-3 text-[10px] leading-relaxed text-slate-400">{suplaAiPricing.pricing_rule} {suplaAiPricing.architecture_note} {suplaAiPricing.software_note}</p>
              </div>}
            </div>
          </div>
          ...
          {prepared && (
            <div id="offer-review" className="mt-6 scroll-mt-24 rounded-2xl border border-cyan-300 bg-cyan-50/50 p-5 sm:p-6">
              ...
              <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-mono uppercase tracking-widest text-cyan-800">Kompletní nabídka · připravena jako koncept</p><p className="mt-1 text-sm font-semibold text-slate-950">{prepared.quoteNumber} · platnost do {prepared.validUntil.toLocaleDateString('cs-CZ')}</p></div><CheckCircle2 size={20} className="text-cyan-700"/></div>
              <div className="mt-4 grid gap-2 sm:grid-cols-4">
                <div className="rounded-xl border border-emerald-200 bg-white px-3 py-3 text-xs font-semibold text-emerald-800">✓ Doporučené řešení</div>
                <div className={`rounded-xl border bg-white px-3 py-3 text-xs font-semibold ${prepared.visualizationUrls?.length ? 'border-emerald-200 text-emerald-800' : 'border-amber-200 text-amber-800'}`}>{prepared.visualizationUrls?.length ? '✓ Vizualizace' : '! Vizualizace chybí'}</div>
                <div className={`rounded-xl border bg-white px-3 py-3 text-xs font-semibold ${prepared.quoteDriveUrl ? 'border-emerald-200 text-emerald-800' : 'border-rose-200 text-rose-800'}`}>{prepared.quoteDriveUrl ? '✓ PDF na Mlžném disku' : '! PDF není uložené'}</div>
                <div className="rounded-xl border border-emerald-200 bg-white px-3 py-3 text-xs font-semibold text-emerald-800">✓ Koncept zprávy</div>
              </div>
          ...
          <div className="mt-6 grid gap-3">
            <label className="text-xs font-semibold text-muted-foreground">Předmět zprávy · koncept
            ...
            <label className="text-xs font-semibold text-muted-foreground">Zpráva klientovi · koncept (nic se automaticky neodesílá)
