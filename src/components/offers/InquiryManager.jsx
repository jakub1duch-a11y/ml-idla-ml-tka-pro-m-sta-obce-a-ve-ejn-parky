          ...
          <div className="rounded-2xl border border-cyan-200 bg-cyan-50/40 p-5">
            ...
            <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-5">
              ...
              {suplaAiPricing?.ok && <div className="mt-4">
                <div className="flex flex-wrap gap-2 text-[10px] text-slate-500"><span className="rounded-full bg-slate-100 px-2.5 py-1">Sazba programování: <strong className="text-slate-800">{money(suplaAiPricing.hourly_rate_ex_vat)} Kč/h</strong></span><span className="rounded-full bg-slate-100 px-2.5 py-1">Zdroj: {suplaAiPricing.source}</span></div>
                <div className="mt-4 grid gap-3 lg:grid-cols-3">{(suplaAiPricing.phases || []).map((phase) => <div key={phase.key} className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-[10px] font-semibold uppercase tracking-wider text-cyan-700">{phase.weeks_min}–{phase.weeks_max} týdny · {phase.hours_min}–{phase.hours_max} h</p><h4 className="mt-2 text-sm font-bold text-slate-900">{phase.title}</h4><p className="mt-2 text-[11px] leading-5 text-slate-500">{phase.scope}</p><p className="mt-3 text-sm font-bold text-[#0e5b67]">{money(phase.price_min_ex_vat)}–{money(phase.price_max_ex_vat)} Kč bez DPH</p></div>)}</div>
                <div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-cyan-100 bg-cyan-50 p-3"><p className="text-[9px] uppercase tracking-wider text-cyan-700">Vývoj celkem</p><p className="mt-1 text-sm font-bold text-cyan-950">{money(suplaAiPricing.software_total_min_ex_vat)}–{money(suplaAiPricing.software_total_max_ex_vat)} Kč</p></div><div className="rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="text-[9px] uppercase tracking-wider text-slate-500">Kompletní SUPLA HW</p><p className="mt-1 text-sm font-bold text-slate-900">{money(suplaAiPricing.complete_supla_hardware_ex_vat)} Kč</p></div><div className="rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="text-[9px] uppercase tracking-wider text-slate-500">Vývoj + HW</p><p className="mt-1 text-sm font-bold text-slate-900">{money(suplaAiPricing.total_with_complete_hardware_min_ex_vat)}–{money(suplaAiPricing.total_with_complete_hardware_max_ex_vat)} Kč</p></div></div>
                <p className="mt-3 text-[10px] leading-relaxed text-slate-400">{suplaAiPricing.pricing_rule} {suplaAiPricing.hardware_note}</p>
              </div>}
            </div>
          </div>
