  ...
  return <div className="overflow-hidden rounded-[28px] border border-white/10 bg-[#071b25] text-white shadow-2xl shadow-black/10">
    ...
    <div className="grid lg:grid-cols-[.92fr_1.08fr]">
      <div className="border-b border-white/10 p-6 sm:p-8 lg:border-b-0 lg:border-r">
        <label className="font-mono text-[10px] uppercase tracking-[.18em] text-white/45">Mlžný systém</label>
        <div className="relative mt-2">
          <button onClick={()=>setOpen(!open)} className="flex w-full items-center justify-between rounded-2xl border border-white/15 bg-white/[.04] px-5 py-4 text-left transition hover:border-cyan-300/40"><span><strong className="block text-sm">{product.name}</strong><small className="mt-1 block text-white/45">{product.range} · {product.pressure}</small></span><ChevronDown size={17} className={`text-white/50 transition ${open?'rotate-180':''}`}/></button>
      ...
      <div className="p-6 sm:p-8">
        ...
        <div className="mt-3 rounded-2xl border border-cyan-300/25 bg-gradient-to-br from-cyan-300/10 to-transparent p-6">
          ...
          <div className="mt-4 flex items-end gap-2"><strong className="font-heading text-5xl font-light tabular-nums">{money(calc.cost)}</strong><span className="pb-1 text-white/50">Kč / měsíc</span></div>
          <p className="mt-3 text-xs leading-relaxed text-white/45">Počítáno orientačně s vodným a stočným {number(WATER_PRICE,2)} Kč/m³. Skutečná sazba se liší podle lokality a provozovatele.</p>
        </div>
        {smart && <div className="mt-3 flex items-center justify-between rounded-2xl border border-white/10 bg-white/[.035] p-5"><div><p className="text-xs text-white/45">Modelovaná úspora díky chytrému řízení</p><p className="mt-1 text-sm text-white/70">oproti nepřetržitému provozu v nastaveném čase</p></div><strong className="ml-4 whitespace-nowrap text-xl text-cyan-200">≈ {money(calc.saving)} Kč/měs.</strong></div>}
        <div className="mt-5 grid grid-cols-2 gap-3 border-t border-white/10 pt-5 text-sm"><div><span className="block text-xs text-white/40">Průtok produktu</span><strong className="mt-1 block">{product.range}</strong></div><div><span className="block text-xs text-white/40">Provozní tlak</span><strong className="mt-1 block">{product.pressure}</strong></div></div>
      </div>
    </div>
  </div>;
}
