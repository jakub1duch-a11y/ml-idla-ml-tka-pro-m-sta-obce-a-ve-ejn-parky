...
export default function ProductGalleryPanel({ images, productName, onOpenLightbox }) {
  ...
  const visibleThumbs = safeImages.slice(0, 12);
  const secondaryIndexes = safeImages.length > 1
    ? [1, 2].map((offset) => (active + offset) % safeImages.length).filter((index, pos, list) => index !== active && list.indexOf(index) === pos)
    : [];
      ...
      <div className="overflow-hidden rounded-[30px] border border-slate-200/80 bg-white p-2.5 shadow-[0_26px_72px_rgba(15,23,42,.09)] sm:p-3">
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1.62fr)_minmax(172px,.58fr)]">
          <div
            ...
            aria-label={`Otevřít fotografii ${active + 1} z ${safeImages.length} v galerii`}
          ...
          >
            <AnimatePresence mode="wait" initial={false}>
              <motion.div key={`backdrop-${safeImages[active]}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: reduceMotion ? 0 : .28 }} className="absolute inset-0">
                <img src={safeImages[active]} alt="" aria-hidden="true" className="h-full w-full scale-110 object-cover opacity-[.12] blur-2xl" />
        ...
        {safeImages.length > 1 && <div className="mt-3">
          ...
          <div className="flex snap-x snap-mandatory gap-2 overflow-x-auto pb-1.5 pr-1 [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: 'none' }}>
            {visibleThumbs.map((img, i) => <button
              ...
              className={`relative aspect-[4/3] min-w-[78px] snap-start overflow-hidden rounded-xl border bg-[#f7f9f9] transition-all duration-300 sm:min-w-[94px] ${active === i ? 'border-[#0b4860] shadow-[0_6px_18px_rgba(11,72,96,.14)] ring-2 ring-[#0b4860]/10' : 'border-slate-200 opacity-80 hover:border-slate-300 hover:opacity-100'}`}
            >
              ...
              {i === visibleThumbs.length - 1 && safeImages.length > visibleThumbs.length && <span className="absolute inset-0 flex items-center justify-center bg-[#062d3a]/[.74] text-xs font-semibold text-white backdrop-blur-sm">+{safeImages.length - visibleThumbs.length}</span>}
            </button>)}
          </div>
        </div>}
      </div>
