  ...
  return (
    <Link to={`/produkt/${product.slug}`} className="group block h-full overflow-hidden rounded-2xl border border-slate-200 bg-white transition-all duration-300 hover:-translate-y-1 hover:border-secondary hover:shadow-xl">
      ...
      <div className="p-5">
        ...
        {isNozzle && <div className="mt-4 border-t border-slate-100 pt-4"><p className="text-[10px] font-bold uppercase tracking-widest text-secondary">Varianty</p><div className="mt-2 flex gap-2">{['M1', 'M2 · standard', 'M3'].map((item) => <span key={item} className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${item.startsWith('M2') ? 'bg-primary text-primary-foreground' : 'bg-slate-100 text-slate-500'}`}>{item}</span>)}</div><p className="mt-3 text-[11px] text-slate-500">{product.pressure} · {product.water_consumption}</p></div>}
      </div>
    </Link>
  );
}