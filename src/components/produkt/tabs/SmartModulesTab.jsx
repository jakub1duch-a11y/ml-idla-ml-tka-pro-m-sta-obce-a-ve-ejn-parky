...
import { Cloud, Smartphone, Gauge, Droplet, Sun, Thermometer } from 'lucide-react';
...
const ALL_MODULES = [
  { icon: Smartphone, title: 'HolmApp řízení', desc: 'Ovládání mlžení odkudkoliv z mobilu, notifikace o stavu systému.', match: () => true },
  { icon: Cloud, title: 'Předpověď v cloudu', desc: 'Automatická regulace dle teploty, vlhkosti a předpovědi počasí.', match: () => true },
  { icon: Droplet, title: 'Monitoring nádrží', desc: 'Snímač hladiny vody chrání čerpadlo proti chodu na sucho.', match: (p) => !!p.water_consumption },
  { icon: Gauge, title: 'Regulace tlaku', desc: '2–7 BAR — napojení na běžný vodovodní řad bez čerpadel.', match: (p) => !!p.pressure },
  { icon: Sun, title: 'Solární napájecí modul', desc: 'Nezávislý provoz mimo dosah elektrické sítě.', match: (p) => (p.power_supply || '').toLowerCase().includes('sol') },
  { icon: Thermometer, title: 'Teplotní čidlo', desc: 'Spouští mlžení automaticky při překročení nastavené teploty.', match: () => true },
];

export default function SmartModulesTab({ product }) {
  ...
  const recommended = ALL_MODULES.filter((m) => m.match(product)).slice(0, 4);
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 lg:px-10 py-24 lg:py-32">
      ...
      <p className="text-xs font-mono tracking-widest uppercase text-slate-400 mb-5">Doporučené moduly pro {product.name}</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {recommended.map((m, i) => (
          <motion.div key={m.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}
            className="p-5 rounded-2xl border border-slate-200 bg-slate-50">
            <m.icon size={20} className="text-slate-500 mb-3" />
            <h3 className="text-slate-900 text-sm font-medium mb-1.5">{m.title}</h3>
