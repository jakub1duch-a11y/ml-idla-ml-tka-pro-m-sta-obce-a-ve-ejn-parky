...
export default function RentalInquiryForm({ products }) {
  ...
  const submit = async (event) => { event.preventDefault(); setSending(true); try { const created = await base44.entities.ContactInquiry.create({ name: form.name, email: form.email, project_scope: 'event', message: `[PRONÁJEM / ${form.product || 'výběr modelu'} GO] Akce: ${form.event}. Termín: ${form.date}. ${form.message}`, description: form.phone ? `Tel: ${form.phone}` : '', status: 'new' }); trackRentalInquiry(form.product, form.event, created?.id || ''); base44.analytics.track({ eventName: 'rental_inquiry_submit', properties: { product: form.product || 'nezvoleno', event_type: form.event } }); navigate('/dekujeme?zdroj=pronajem'); } finally { setSending(false); } };
