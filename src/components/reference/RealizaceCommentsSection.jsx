...
export default function RealizaceCommentsSection({ realizaceId }) {
  ...
  const loadComments = () => {
    base44.entities.RealizaceComment.filter({ realizace_id: realizaceId }, '-created_date')
      .then((items) => setComments(items || []))
      .catch(() => {})
      .finally(() => setLoading(false));
  };

  useEffect(() => { if (realizaceId) loadComments(); }, [realizaceId]);

  const handleSubmit = async (e) => {
    e.preventDefault();
    setSending(true);
    await base44.entities.RealizaceComment.create({ realizace_id: realizaceId, ...form });
  ...
  return (
    <div className="py-12 border-t border-slate-200">
      ...
      {loading ? (
        ...
        <p className="text-sm text-slate-400 font-light">Buďte první, kdo okomentuje tuto realizaci.</p>
      ) : (
