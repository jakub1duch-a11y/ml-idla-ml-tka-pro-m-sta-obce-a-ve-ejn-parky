  ...
  return <section className="relative overflow-hidden bg-primary pb-20 pt-28 sm:pt-32 lg:pb-28">
    ...
    <div className="relative mx-auto grid max-w-7xl items-center gap-14 px-6 lg:grid-cols-[1.05fr_.95fr] lg:px-10">
      <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
        ...
        <p className="mt-6 max-w-2xl text-base leading-relaxed text-primary-foreground/70 sm:text-lg">Smart řízení propojí časový režim, teplotu a podle konfigurace další senzory. Jednotlivé zóny se spouštějí jen při splnění nastavených provozních podmínek.</p><p className="mt-3 max-w-xl text-sm leading-relaxed text-primary-foreground/50">Od jednoduchého časového režimu po vzdálenou správu více zón pro města, sportoviště a veřejný prostor.</p>
        ...
        <div className="mt-8 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center"><Link to="/kontakt?produkt=Smart%20ovl%C3%A1d%C3%A1n%C3%AD" className="btn-metallic-mist inline-flex items-center justify-center gap-2 px-7 py-3.5 text-sm font-bold">Poptat chytré řešení <ArrowRight size={16} /></Link><a href="#jak-to-funguje" className="inline-flex items-center justify-center gap-2 rounded-full border border-white/20 px-6 py-3.5 text-sm font-semibold text-white/80 transition hover:border-white/40 hover:text-white">Jak to funguje</a></div>
      </motion.div>
      <div className="relative rounded-3xl border border-white/10 bg-white text-slate-900 shadow-2xl"><motion.img initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.1 }} src="https://media.base44.com/images/public/6a3ee88c10959cd3588c4d68/5c4b99749_Smartmlzitka-ovladanizmobilu.jpg" alt="Ovládání mlžítka z mobilní aplikace" className="aspect-[4/3] w-full rounded-t-3xl bg-white object-contain p-2 opacity-95 sm:p-3" /><div className="relative z-10 mx-3 -mt-12 pb-4 sm:mx-10 sm:-mt-20 sm:pb-5"><AutomationPreview /></div></div>
    </div>
  </section>;
}