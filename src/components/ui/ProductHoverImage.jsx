...
export default function ProductHoverImage({ product, alt = '', className = '', overlay = false, fallback = '' }) {
  ...
  // první odlišná položka galerie = reálná fotografie pro hover.
  ...
  return <div className={`relative overflow-hidden bg-slate-200 ${className}`}>
    ...
    {secondary && <img src={secondary} alt={`${alt || product?.name || 'Produkt'} – reálná fotografie realizace`} loading="lazy" decoding="async" className="absolute inset-0 h-full w-full scale-[1.02] object-cover opacity-0 transition-all duration-700 group-hover:scale-100 group-hover:opacity-100" />}
