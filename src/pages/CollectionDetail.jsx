...
export default function CollectionDetail() {
  ...
  return <main className="bg-background pt-16"><CollectionHero collection={collection} /><section className="max-w-7xl mx-auto px-6 lg:px-10 py-16 lg:py-24 grid gap-10 lg:grid-cols-[.8fr_1.2fr]"><div><p className="font-mono tracking-[.18em] uppercase text-secondary text-sm">NAVRŽENO PRO VAŠE MÍSTO</p><h2 className="mt-4 font-heading text-4xl lg:text-5xl text-foreground">{collection.sectionHeading}</h2></div><div className="grid gap-5 sm:grid-cols-3">{collection.features.map((feature) => <div key={feature} className="border-t border-[#b9c3c8] pt-5"><Check size={18} className="text-secondary" /><p className="mt-6 leading-relaxed text-foreground text-lg">{feature}</p></div>)}</div></section><CollectionProductGrid collection={collection} /><section className="text-primary-foreground bg-[#10223c]"><div className="max-w-7xl mx-auto px-6 lg:px-10 py-16 lg:py-20 flex flex-col lg:flex-row gap-8 items-end justify-between"><div><p className="font-mono text-[11px] tracking-[.18em] uppercase text-accent">Řešení na míru</p><h2 className="mt-4 max-w-2xl font-heading text-4xl lg:text-5xl">Společně vybereme správný model, rozměr i způsob instalace.</h2></div><Link to="/poptavka" className="shrink-0 inline-flex items-center gap-2 bg-accent px-6 py-3 text-sm font-semibold text-accent-foreground">Nezávazná poptávka <ArrowRight size={16} /></Link></div></section></main>;
}