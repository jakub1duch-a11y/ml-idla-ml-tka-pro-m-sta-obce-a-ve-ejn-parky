...
import { Loader, AlertCircle, FileText, CheckCircle, Clock, Download, Share2, MessageSquare, X, Hash, Mail, ShieldCheck, Image, ArrowRight, ExternalLink, Plus, Paperclip, ReceiptText, Shapes, ShoppingBag, UploadCloud, KeyRound, Eye, EyeOff, Sparkles, Send } from 'lucide-react';
                ...
                <div className="px-6 py-4 border-t border-slate-200 space-y-3">
                  ...
                  {['sent','viewed','extension_requested','expired','approved','in_production'].includes(project.status) && (
                    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                      <div className="border-b border-slate-100 bg-gradient-to-r from-slate-50 to-cyan-50/40 px-4 py-4 sm:px-5">
                        <div className="flex items-start gap-3">
                          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#0b4860] text-white"><MessageSquare size={17}/></div>
                          <div><p className="text-[10px] font-mono uppercase tracking-widest text-cyan-700">Chat k projektu</p><h3 className="mt-1 text-base font-semibold text-slate-900">Klient ↔ asistent MLŽIDLA</h3><p className="mt-1 text-xs leading-5 text-slate-500">Pište přímo k této nabídce. Konverzace zůstává uložená u projektu a tým na ni může navázat.</p></div>
                        </div>
                      </div>

                      <div className="max-h-[360px] space-y-3 overflow-y-auto bg-slate-50/60 px-4 py-4 sm:px-5">
                        {(project.offer_messages || []).length === 0 && <div className="rounded-2xl border border-dashed border-slate-200 bg-white px-4 py-5 text-center"><Sparkles size={18} className="mx-auto text-cyan-700"/><p className="mt-2 text-xs font-semibold text-slate-700">Začněte konverzaci k této nabídce</p><p className="mt-1 text-[11px] text-slate-400">Můžete použít některý z návrhů zpráv níže.</p></div>}
                        {(project.offer_messages || []).map((item) => <div key={item.id} className={`flex ${item.sender_type === 'customer' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[88%] rounded-2xl px-4 py-3 ${item.sender_type === 'customer' ? 'rounded-br-md bg-[#0b4860] text-white' : 'rounded-bl-md border border-slate-200 bg-white text-slate-700 shadow-sm'}`}><div className="flex items-center justify-between gap-4"><span className={`text-[9px] font-semibold uppercase tracking-wide ${item.sender_type === 'customer' ? 'text-white/60' : 'text-slate-400'}`}>{item.sender_type === 'customer' ? 'Vy' : 'Asistent MLŽIDLA'}</span><span className={`text-[9px] ${item.sender_type === 'customer' ? 'text-white/45' : 'text-slate-400'}`}>{item.created_date ? new Date(item.created_date).toLocaleString('cs-CZ') : ''}</span></div><p className="mt-1 whitespace-pre-line text-xs leading-5">{item.message}</p></div></div>)}
                      </div>

                      <div className="border-t border-slate-100 bg-white px-4 py-4 sm:px-5">
                        <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-wider text-slate-400"><Sparkles size={13} className="text-cyan-700"/> Návrhy zpráv</div>
                        <div className="mt-2 flex gap-2 overflow-x-auto pb-1">
                          {[
                            ['question', 'Prosím o upřesnění ceny této varianty.'],
                            ['solution_change', 'Chtěl bych upravit počet prvků nebo jejich rozmístění.'],
                            ['technical', 'Prosím o technické upřesnění napojení vody a kotvení.'],
                            ['delivery', 'Jaký je aktuální termín výroby a dodání?'],
                            ['question', 'Můžete mi doporučit nejvhodnější variantu pro tento projekt?']
                          ].map(([category, text]) => <button key={text} type="button" onClick={() => setMessageForms(prev => ({ ...prev, [project.id]: { ...(prev[project.id] || {}), category, message: text } }))} className="shrink-0 rounded-full border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-medium text-slate-600 transition hover:border-cyan-300 hover:bg-cyan-50 hover:text-cyan-900">{text}</button>)}
                        </div>
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          ...
                          ].map(([value,label]) => <button key={value} type="button" onClick={() => setMessageForms(prev => ({ ...prev, [project.id]: { ...(prev[project.id] || {}), category: value } }))} className={`rounded-full border px-3 py-1.5 text-[10px] font-semibold transition ${(messageForms[project.id]?.category || 'question') === value ? 'border-[#0b4860] bg-[#0b4860] text-white' : 'border-slate-200 bg-white text-slate-500 hover:border-slate-300'}`}>{label}</button>)}
                        </div>

                        <div className="mt-3 flex items-end gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-2 focus-within:border-cyan-400 focus-within:bg-white">
                          <textarea value={(messageForms[project.id] || {}).message || ''}
                            ...
                            placeholder="Napište zprávu k projektu…" rows={2}
                            className="min-h-[48px] flex-1 resize-none bg-transparent px-2 py-2 text-sm text-slate-900 outline-none placeholder:text-slate-400" />
                          <button onClick={() => sendOfferMessage(project)} disabled={messageBusy === project.id || !String((messageForms[project.id] || {}).message || '').trim()}
                            className="mb-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#0b4860] text-white transition hover:bg-[#0e6176] disabled:opacity-35" aria-label="Odeslat zprávu">
                            {messageBusy === project.id ? <Loader size={15} className="animate-spin"/> : <Send size={15}/>} 
                          </button>
                        </div>
                        {messageSaved[project.id] && <p className="mt-2 text-[11px] font-medium text-emerald-700">Zpráva byla odeslána a uložena k projektu.</p>}
                      </div>
                    </div>
                  )}
