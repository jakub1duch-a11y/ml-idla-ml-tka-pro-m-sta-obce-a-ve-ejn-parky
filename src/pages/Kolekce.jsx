...
function ProductCard({ product, i }) {
  ...
  return <motion.article initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} className="group flex h-full flex-col overflow-hidden rounded-[20px] border border-border bg-card shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
    ...
    <Link to={`/kontakt?produkt=${encodeURIComponent(product.name)}`} onClick={() => trackQuickInquiryClick(product.name, 'katalog')} className="flex items-center justify-center gap-1.5 border-t border-border py-3 text-xs font-bold text-foreground transition hover:bg-muted bg-[hsl(var(--muted-foreground))]"><Zap size={13} /> Popsat projekt</Link>
  </motion.article>;
}
