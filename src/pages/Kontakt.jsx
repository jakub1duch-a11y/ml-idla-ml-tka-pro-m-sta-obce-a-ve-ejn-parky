...
{ icon: MapPin, label: 'Adresa', value: "541 02 Trutnov, Horn\xED Star\xE9 M\u011Bsto 698, \u010Cesk\xE1 republika", href: null }];
...
export default function Kontakt() {
  ...
  const inputCls = "w-full px-4 py-3.5 rounded-xl bg-white border border-slate-200 text-slate-900 text-sm placeholder-slate-400 focus:border-slate-400 focus:outline-none transition-all";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            className="p-6 rounded-2xl bg-slate-50 border border-slate-200 flex items-center gap-4">
                ...
                <div>
                  ...
                  {item.href ?
                <a href={item.href} className="text-slate-900 font-medium hover:text-slate-600 transition-colors">{item.value}</a> :
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-3">
            <form onSubmit={handleSubmit} className="p-8 rounded-2xl bg-slate-50 border border-slate-200 space-y-5">
              ...
              className="btn-inquiry-pulse w-full rounded-full bg-primary py-4 justify-center text-sm font-bold text-primary-foreground disabled:opacity-50">
