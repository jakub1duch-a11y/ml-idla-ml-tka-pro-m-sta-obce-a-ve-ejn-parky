...
export default function ObchodniNabidky() {
  ...
  const downloadOffer = async (product) => {
    ...
    const response = await base44.functions.invoke('generateProductDatasheet', {
      ...
      ai_content: { presentation_title: profile?.offer_headline || `${product.name} — produktová nabídka`, solution_summary: profile?.offer_summary || product.short_description || '' },
    });
  ...
  return <main className="min-h-screen bg-background pt-16">
    <section className="bg-primary text-white"><div className="mx-auto max-w-7xl px-6 py-14 lg:px-10 lg:py-16"><div className="flex flex-wrap items-start justify-between gap-5"><div><p className="flex items-center gap-2 font-mono text-[11px] tracking-[.18em] uppercase text-accent"><LockKeyhole size={14}/> Interní obchodní centrum</p><h1 className="mt-4 font-heading text-4xl lg:text-6xl">MLŽIDLA Sales Hub</h1><p className="mt-5 max-w-2xl text-lg text-white/70">Profesionální architektonické nabídky pro mlžné instalace: vizualizace, kalkulace, PDF, Google Drive, Gmail koncept a klientský portál na jednom místě.</p><div className="mt-5 flex flex-wrap gap-2 text-[10px] font-semibold uppercase tracking-[.14em] text-white/55"><span className="rounded-full border border-white/15 px-3 py-1.5">Architektonické PDF</span><span className="rounded-full border border-white/15 px-3 py-1.5">Mlžný disk</span><span className="rounded-full border border-white/15 px-3 py-1.5">Gmail koncept</span><span className="rounded-full border border-white/15 px-3 py-1.5">Schválení před odesláním</span></div></div><a href="/admin" className="rounded-full border border-white/20 px-4 py-2 text-xs font-semibold text-white/75 hover:bg-white/10">← Administrace</a></div><div className="mt-9 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-2xl border border-white/10 bg-white/5 p-4"><div className="text-[10px] uppercase tracking-widest text-white/45">Poptávky</div><div className="mt-2 text-2xl font-semibold">{inquiries.length}</div></div><div className="rounded-2xl border border-white/10 bg-white/5 p-4"><div className="text-[10px] uppercase tracking-widest text-white/45">Aktivní nabídky</div><div className="mt-2 text-2xl font-semibold">{openOffers}</div></div><div className="rounded-2xl border border-white/10 bg-white/5 p-4"><div className="text-[10px] uppercase tracking-widest text-white/45">Objednáno</div><div className="mt-2 text-2xl font-semibold">{approvedOrders}</div></div><div className="rounded-2xl border border-white/10 bg-white/5 p-4"><div className="text-[10px] uppercase tracking-widest text-white/45">Hodnota pipeline</div><div className="mt-2 text-2xl font-semibold">{formatPrice(pipelineValue)}</div></div></div></div></section>
