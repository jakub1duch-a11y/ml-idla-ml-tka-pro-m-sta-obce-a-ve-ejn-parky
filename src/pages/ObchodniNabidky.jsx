...
export default function ObchodniNabidky() {
  ...
  useEffect(() => { if (isAuthenticated) Promise.all([base44.entities.Product.list(), base44.entities.MediaFile.list(), loadInquiries()]).then(([productData, fileData]) => { setProducts(productData); setMediaFiles(fileData); }).finally(() => setLoading(false)); }, [isAuthenticated]);
  ...
  return <main className="min-h-screen bg-background pt-16">
    ...
    {!loading && <InquiryManager inquiries={inquiries} products={products} mediaFiles={mediaFiles} onSent={loadInquiries}/>} 
