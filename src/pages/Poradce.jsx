      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
          <div className="w-14 h-14 rounded-2xl bg-cyan/10 border border-cyan/20 flex items-center justify-center mx-auto mb-5">
            <Droplets size={26} className="text-cyan" />
          </div>
          <p className="text-xs font-mono tracking-widest uppercase text-cyan mb-3">AI Poradce</p>
        ...
        {/* Chat area */}
        <div className="bg-card_bg border border-white/10 rounded-2xl overflow-hidden flex flex-col" style={{ minHeight: '520px' }}>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4" style={{ maxHeight: '520px' }}>
            {messages.length === 0 && !starting && (
              <div className="h-full flex flex-col items-center justify-center gap-6 py-8">
                <p className="text-white/30 text-sm text-center">Zahajte konverzaci nebo vyberte téma:</p>
                <div className="flex flex-col gap-2 w-full max-w-sm">
                  {SUGGESTED_QUESTIONS.map((q) => (
                    <button key={q} onClick={() => handleSuggestion(q)}
                      className="text-left px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white/60 text-sm hover:bg-cyan/10 hover:border-cyan/30 hover:text-white transition-all">
                      {q}
                    </button>
                  ))}
                </div>
            ...
            {starting && (
              <div className="flex justify-center py-8">
                <Loader size={20} className="animate-spin text-cyan/50" />
              </div>
            )}
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role !== 'user' && (
                  <div className="w-7 h-7 rounded-full bg-cyan/10 border border-cyan/20 flex items-center justify-center flex-shrink-0 mr-2 mt-1">
                ...
                <div className={`max-w-[80%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                  msg.role === 'user'
                    ...
                    : 'bg-surface border border-white/10 text-white/80'
                }`}>
                  {msg.role === 'user' ? (
                    <p>{msg.content}</p>
                  ) : (
            ...
            {isAssistantTyping && (
              <div className="flex justify-start items-center gap-2">
                ...
                <div className="bg-surface border border-white/10 px-4 py-3 rounded-2xl flex gap-1.5 items-center">
          ...
          <div className="border-t border-white/10 p-4 flex gap-3">
            <input
              ...
              className="flex-1 px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white text-sm placeholder-white/25 focus:border-cyan/40 focus:outline-none disabled:opacity-50"
            />
            <button onClick={sendMessage} disabled={!input.trim() || loading || starting}
              className="w-11 h-11 rounded-xl bg-cyan text-ink flex items-center justify-center hover:bg-cyan/90 disabled:opacity-40 transition-all flex-shrink-0">
