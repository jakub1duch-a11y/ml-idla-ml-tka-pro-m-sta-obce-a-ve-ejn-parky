...
export default function ReferenceDetail({ fixedId }) {
 ...
 <section className="bg-slate-50 border-y border-slate-200"><div className="max-w-7xl mx-auto px-6 lg:px-10 py-14 lg:py-20 grid lg:grid-cols-2 gap-12"><div><p className="font-mono text-[11px] tracking-[.18em] uppercase text-teal-700 mb-3">{isZoo ? 'Veřejný prostor' : 'Návrh a realizace'}</p><h2 className="font-heading text-3xl lg:text-4xl">{isZoo ? 'Reference, která prověřuje řešení každý den.' : 'Od návrhu k funkčnímu ochlazení prostoru.'}</h2></div><p className="text-lg leading-relaxed text-slate-600">{isZoo ? 'Zoologická zahrada je prostředí s vysokou návštěvností, dětmi, rodinami a dlouhými pěšími trasami. Právě zde je důležitá kombinace spolehlivosti, bezpečného provozu, snadné údržby a skutečného komfortu pro návštěvníka.' : `Každou realizaci řešíme podle charakteru místa, očekávaného provozu a způsobu instalace. U projektu ${project.name} jsme spojili čisté nerezové provedení s funkčním rozmístěním mlžicích prvků tak, aby výsledné řešení přirozeně zapadlo do prostoru a bylo dlouhodobě snadno provozovatelné.`}</p></div></section>
