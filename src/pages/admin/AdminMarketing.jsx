...
import { Loader, CalendarClock, Palette, Sparkles, Gauge, Megaphone, CheckCircle2, Clock3, Layers3, ImageIcon, ShieldCheck } from 'lucide-react';
...
const SUBTABS = [
  ...
  { id:'thumbnails', label:'Miniatury', sub:'A/B/C koncepty a 50% test', icon:ImageIcon },
  { id:'visuals', label:'Produktové vizualizace', sub:'MASTER lock · města, eventy a zahrady', icon:ShieldCheck },
...
export default function AdminMarketing(){
  ...
  const loadProducts=()=>{setProductsLoading(true);base44.entities.Product.list('name',250).then((rows)=>setProducts((rows||[]).filter((p)=>p?.slug&&!p.slug.startsWith('archived-')&&p.slug!=='mlzici-tryska'))).finally(()=>setProductsLoading(false));};
  useEffect(()=>{loadPosts();loadProducts();},[]);
  ...
  return <div className="space-y-6 p-6">
    ...
    {subtab==='thumbnails'&&<ThumbnailDirectorTab/>}
    {subtab==='visuals'&&(productsLoading?<div className="flex justify-center py-16"><Loader size={22} className="animate-spin text-cyan/40"/></div>:<VisualizationStudio products={products}/>)}
