...
const EMPTY = {
  ...
  subtype: '', tool: '', connector: '', related_type: '', related_label: '', related_url: '', handoff_note: '', result_summary: '', evidence: '', tags: []
};
...
export default function AdminTasks({ embedded = false }) {
  ...
  const syncCompletedWorkLog = async (task) => {
    ...
    const payload = {
      ...
      subtype: task.subtype || task.area || 'interní úkol',
      is_estimate: false,
      tool: task.tool || (task.source === 'chatgpt' ? 'ChatGPT + Base44' : 'Admin Úkoly & tým'),
      connector: task.connector || task.related_type || '',
      task_id: task.id,
    };
  ...
  return <div className={embedded ? 'space-y-5' : 'p-6 space-y-6'}>
    ...
    {editing && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4"><div className="max-h-[92vh] w-full max-w-3xl overflow-auto rounded-2xl border border-white/10 bg-[#0d1117] p-5 shadow-2xl"><div className="flex items-center justify-between"><div><p className="font-mono text-[10px] uppercase tracking-widest text-cyan">{editing === 'new' ? 'Nový úkol' : 'Editace úkolu'}</p><h3 className="mt-1 text-lg text-white">{editing === 'new' ? 'Zadat práci' : form.title}</h3></div><button onClick={()=>setEditing(null)} className="text-white/30 hover:text-white"><X size={18}/></button></div>
      ...
      <label className="text-xs text-white/35">Oblast<select value={form.area} onChange={e=>setForm(f=>({...f,area:e.target.value}))} className="mt-1 block w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2.5 text-sm text-white">{AREAS.map(a=><option key={a} value={a}>{a}</option>)}</select></label><label className="text-xs text-white/35">Podtyp práce<input value={form.subtype || ''} onChange={e=>setForm(f=>({...f,subtype:e.target.value}))} placeholder="UX/UI, video, nabídka, promptování…" className="mt-1 block w-full rounded-lg border border-white/10 bg-black/15 px-3 py-2.5 text-sm text-white"/></label>
      <label className="text-xs text-white/35">Nástroj<input value={form.tool || ''} onChange={e=>setForm(f=>({...f,tool:e.target.value}))} placeholder="ChatGPT, Base44, GitHub…" className="mt-1 block w-full rounded-lg border border-white/10 bg-black/15 px-3 py-2.5 text-sm text-white"/></label><label className="text-xs text-white/35">Konektor / služba<input value={form.connector || ''} onChange={e=>setForm(f=>({...f,connector:e.target.value}))} placeholder="Gmail, Ads, Drive, HYPD…" className="mt-1 block w-full rounded-lg border border-white/10 bg-black/15 px-3 py-2.5 text-sm text-white"/></label>
      <label className="text-xs text-white/35">Termín<input type="date" value={form.due_date || ''} onChange={e=>setForm(f=>({...f,due_date:e.target.value}))} className="mt-1 block w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2.5 text-sm text-white"/></label>
